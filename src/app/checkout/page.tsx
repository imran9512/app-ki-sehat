  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const url =
      ...
      Object.entries({
        ...
        [ENTRY_IDS.products_json]: rows.map((r) => `${r.name}×${r.qty}`).join(", "),
    ...
    new Image().src = url.replace("viewform", "formResponse");

    clearCart();
    setForm({ name: "", number: "", city: "", address: "", instructions: "" });
    setShowModal(true);
  };

  const orderId = Date.now().toString();   // already defined above
        ...
        {rows.length === 0 ? (
          ...
          <>
            ...
            <ul className="mb-4 space-y-1">
              {rows.map((r) => (
                <li key={r.id}>
                  {r.name} ×{r.qty} = {CURRENCY}{r.qty * r.priceEach}
                </li>
              ))}
            </ul>
            ...
            {payment === "ADVANCE" && (
              <p className="text-green-600">
                5 % advance discount: ‑{CURRENCY}{discount.toFixed(0)}
              </p>
            )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded p-6 max-w-sm w-full text-center">
            ...
            {payment === "ADVANCE" && (
              <a href={whatsAppLink} target="_blank" rel="noopener" className="inline-block bg-[#25D366] text-white px-4 py-2 rounded">
                Chat on WhatsApp
              </a>
            )}
