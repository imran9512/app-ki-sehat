      ...
      {rows.length === 0 ? (
        ...
        <>
          <div className="space-y-4">
            {rows.map((p) => (
              <div key={p.id} className="flex gap-4 border p-2 rounded">
                <Image src={`/images/${p.images[0]}`} alt={p.name} width={80} height={80} className="object-cover rounded" />
                <div>
                  <h2 className="font-bold">{p.name}</h2>
                  <p>{CURRENCY}{p.priceEach} each</p>
                  <input type="number" min="1" value={p.qty} onChange={(e) => update(p.id, +e.target.value)} className="w-16 border px-1" />
                  <button onClick={() => remove(p.id)} className="ml-4 text-red-600">Remove</button>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-6 text-right">
            ...
            <a href="/checkout" className="inline-block mt-2 bg-green-600 text-white px-4 py-2 rounded">Checkout</a>
          </div>
        </>
      )}
    </div>
